...
export const useTabManager = () => {
  ...
  useEffect(() => {
    ...
    const cleanup2 = electronAPI.ipc.on('tab-loading-changed', (_, tabId: string, loading: boolean) => {
      setIsLoading(prev => ({
        ...prev,
        [tabId]: loading
      }))
    ...
    const cleanup3 = electronAPI.ipc.on('tab-title-changed', (_, tabId: string, title: string) => {
      setTabs(prevTabs => 
        prevTabs.map(tab => 
          tab.id === tabId ? { ...tab, title, updatedAt: new Date() } : tab
        )
      )
    })
  ...
  // 创建新标签页
  const createTab = useCallback(async (options: TabCreateOptions): Promise<TabItem | null> => {
    try {
  ...
  // 获取标签页加载状态
  const getTabLoadingState = useCallback((tabId: string) => {
