...
import { TabItem, useTabManager } from '../hooks/useTabManager'
...
export const TabRenderer: React.FC<TabRendererProps> = ({
  ...
  const handleDragStart = useCallback(
    (event: React.DragEvent, tabId: string, index: number) => {
    ...
    (tab: TabItem, index: number) => {
      ...
      return (
        <div
          ...
          draggable={enableReordering ? !isUserCenter : false}
        ...
        >
          ...
          {/* 标签页标题 */}
          <div className='tab-item__title'>{tab.title || '新标签页'}</div>
      ...
      <div className={`tab-renderer ${className}`}>
        ...
        >
          ...
          {tabs.length > 0 && tabs.length < maxTabs && (
            <button className='tab-list__add-button' onClick={handleCreateTab} title='新建标签页'>
