...
const detectLoopPattern = (): boolean => {
  ...
  // 清理过期记录
  navigationHistory = navigationHistory.filter(record => 
    now - record.timestamp < LOOP_DETECTION_WINDOW
  )
  ...
  for (let i = 0; i < navigationHistory.length - 2; i++) {
    ...
    if (current && next && afterNext) {
      if (
        (current.type === 'gotoLogin' && next.type === 'gotoMain' && afterNext.type === 'gotoLogin') ||
  ...
  const recentSameType = navigationHistory.filter(
    record => record.type === navigationHistory[navigationHistory.length - 1]?.type && 
...
const authAPI = {
    ...
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && (key.includes('token') || key.includes('auth') || 
                   key.includes('login') || key.includes('user'))) {
  ...
  clearLoginCookies: (): Promise<any> => 
    ipcRenderer.invoke('view:clear-login-cookies'),
  
  clearAllLoginState: (): Promise<any> => 
    ipcRenderer.invoke('view:clear-all-login-state')
}

// 配置管理 API
const configAPI = {
  getUserConfig: (): Promise<any> => 
    ipcRenderer.invoke('view:get-user-config'),
...
const loginStateAPI = {
  getCurrentState: (): Promise<any> => 
    ipcRenderer.invoke('view:login-state-get'),
  
  isLoggedIn: (): Promise<boolean> => 
    ipcRenderer.invoke('view:login-state-is-logged-in'),
  
  getUserInfo: (): Promise<any> => 
    ipcRenderer.invoke('view:login-state-get-user-info'),
  
  logout: (): Promise<any> => 
    ipcRenderer.invoke('view:login-state-logout'),
  
  getStats: (): Promise<any> => 
    ipcRenderer.invoke('view:login-state-get-stats'),
